...
function initializeBorders() {
  ...
  for (let i = 0; i < numBorders; i++) {
    ...
    // Create colourful small dot border
    let border = new CircleBorder(x, y - yOffset, 70, 40, 40); // Set spacing to 40, increase gaps
    circleBorders.push(border);

    // Create black small dot border
    let border2 = new CircleBorder2(x, y - yOffset, 65, 12, 40); // Black dot position offset 10
...
class CircleBorder {
  constructor(x, y, bigRadius, numDots, spacing) {
    ...
    this.numDots = numDots; // Number of colourful small dots
  ...
  generateDots() {
    ...
    for (let i = 0; i < this.numDots; i++) {
      ...
      let dotStrokeWeight = 2; // Fixed stroke width
  ...
  drawDot(x, y, radius, strokeWeightValue, color) {
    ...
    strokeWeight(strokeWeightValue); // Fixed stroke width
    fill(color); // Random color fill
...
class CircleBorder2 {
  constructor(x, y, bigRadius, numDots, offset) {
    ...
    this.y = y;
    this.bigRadius = bigRadius;
    this.numDots = numDots; // Number of black small dots
    this.offset = offset;
    this.dots = this.generateDots();
  }
  ...
  drawDot(x, y, radius, strokeWeightValue) {
    stroke(22, 79, 92); // Border color is orange
    strokeWeight(3* scale); // Fixed stroke width
    fill(0); // Black fill
